<?php
	class WPMK_Cache{
        ...
        private $Menu_Title = "WPMK Cache";
		private $Page_Title = "WPMK Cache Settings";
		...
		if(is_file(ABSPATH.".htaccess") && is_writable(ABSPATH.".htaccess")){
		    ...
		    $htaccess = preg_replace("/#\s?BEGIN\s?WPMKCache.*?#\s?END\s?WPMKCache/s", "", $htaccess);
		    $htaccess = preg_replace("/#\s?BEGIN\s?GzipWPMKCache.*?#\s?END\s?GzipWPMKCache/s", "", $htaccess);
		    file_put_contents(ABSPATH.".htaccess", $htaccess);
			}

			wp_clear_scheduled_hook("wp_cache");
			delete_option("WPCache");
			$wpcache = new WPMK_Cache();
			$wpcache->deleteCache();
		}

		public function OptionsPanelRequest(){
			if(!empty($_POST)){
				if(isset($_POST["WPMKCache_Page"])){
					if($_POST["WPMKCache_Page"] == "Options"){
						$this->saveOption();
					}else if($_POST["WPMKCache_Page"] == "deleteCache"){
						$this->deleteCache();
					}else if($_POST["WPMKCache_Page"] == "cacheTimeout"){
		 ...
		 *
		 * Author: Mubeen Khan
		 * URI: http://www.gensoftgroup.com/
		 * Email: info@gensoftgroup.con
		 */
		...
		public function register_WPCache_menu(){
			if(function_exists('add_menu_page')){ 
                add_menu_page( __( 'WPMK Cache', 'wpcache' ), __( 'WPMK Cache', 'wpcache' ), 'manage_options', __FILE__, array($this, 'OptionsPage'));
			}
		}
        ...
        ?>
            ...
            <div class="wrap">
                <div id="icon-options-general" class="icon32"><br /></div>
                    <h2><?php _e( 'WPMK Cache Options', 'wpcache'); ?></h2>
                
                <?php if($this->System_Message){ ?>
                <div class="updated <?php echo $this->System_Message[1]; ?>" id="message">
                    <p><?php echo $this->System_Message[0]; ?></p>
                </div>
                <?php } ?>
                
                <form method="post" name="wp_manager"><!-- General Options : begin -->
                    <input type="hidden" value="Options" name="WPMKCache_Page" />
                    <div id="poststuff">
                        <div class="postbox">
                            <table class="form-table">
                                <tbody>
                                    <h3 style="margin-left: 10px;"><?php _e( 'General Options', 'wpcache' ); ?></h3>
                                    
                                    <tr valign="top">
                                        <th scope="row">
                                            <label for="home">&nbsp;&nbsp;&nbsp;<b><?php _e( 'Cache Frontend', 'wpcache' ); ?></b></label>
                                        </th>
                                    ...
                                    <tr valign="top">
                                        <th scope="row">
                                            <label for="home">&nbsp;&nbsp;&nbsp;<b><?php _e( 'New Post or Page', 'wpcache' ); ?></b></label>
                                        </th>
                ...
                <form method="post" name="wp_manager"><!-- Clear Cache : begin -->
                    <input type="hidden" value="deleteCache" name="WPMKCache_Page" />
                    <div id="poststuff">
                        <div class="postbox">
                            <table class="form-table">
                                <tbody>
                                    <h3 style="margin-left: 10px;"><?php _e( 'Delete Cache', 'wpcache' ); ?></h3>
                                    
                                    <tr valign="top">
                                        <th scope="row">
                                            <label for="home">&nbsp;&nbsp;&nbsp;<b><?php _e( 'Clear all cache', 'wpcache' ); ?></b></label>
                                        </th>
                                        
                                        <td>
                                            <label for="WPCache_Delete_All_Cache">
                                                <i><?php _e( 'Target folder:', 'wpcache' ); ?></i><br />
                                                <pre style="margin-top:10px;background:#FFFFFF;padding-left:10px;padding-top:15px;border: 1px dashed #CCCCCC;">
                                                    <b style="float: left;"><?php echo $this->WP_con_DIR.'/cache/'.$this->WP_blog_Path; ?></b>
                                                </pre>
                                            </label>
                                        </td>
		...
		public function modifyHtaccess($post){
			if(isset($post["WPCache_Status"]) && $post["WPCache_Status"] == "on"){
				if(!is_file(ABSPATH.".htaccess")){
					...
					//$htaccess = file_get_contents(ABSPATH.".htaccess");
					$htaccess = $this->insertRewriteRule($htaccess = null );
		...
		public function insertGzipRule($htaccess, $post){
			if(isset($post["WPCache_Gzip"]) && $post["WPCache_Gzip"] == "on"){
		        ...
		        $gzip[] = "<IfModule mod_deflate.c>";
		  		...
		  		$gzip[] = "</IfModule>";
                //insert_with_markers( ABSPATH.".htaccess", "GzipWPMKCache", $gzip );
			}else{
				//delete gzip rules
				insert_with_markers( ABSPATH.".htaccess", "WPMKCache", $htaccess );
			}
		}
		...
		public function cacheDate($buffer){
			return $buffer."<!-- WPMK Cache generated this file in ".$this->creationTime()." seconds, on ".date("m-d-y G:i:s")." -->";
		}
